  ...
  endpoints: (builder) => ({
    ...
    getProduct: builder.query({
      ...
      providesTags: (result, error, id) => [
        { type: 'Product', id },
        ...(result?.variants?.map((v: any) => ({ type: 'Variant' as const, id: v.id })) || []),
      ],
    }),
    ...
    updateVariant: builder.mutation({
      ...
      async onQueryStarted({ productId, id, ...body }, { dispatch, queryFulfilled }) {
        const patchResult = dispatch(
          productsApi.util.updateQueryData('getProduct', productId, (draft) => {
            const variantIndex = draft.variants.findIndex((v: any) => v.id === id);
            if (variantIndex !== -1) {
    ...
    deleteVariant: builder.mutation({
      ...
      async onQueryStarted({ id, productId }, { dispatch, queryFulfilled }) {
        const patchResult = dispatch(
          productsApi.util.updateQueryData('getProduct', productId, (draft) => {
            draft.variants = draft.variants.filter((v: any) => v.id !== id);
          })
        );
        try {
          await queryFulfilled;
