...
export const reportsApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        getDailyReport: builder.query({
            ...
            providesTags: (result, error, date) => [{ type: 'Report' as const, id: `daily-${date}` }],
        }),

        // Get monthly report
        getMonthlyReport: builder.query({
            query: ({ year, month }) => `/reports/monthly?year=${year}&month=${month}`,
            providesTags: (result, error, { year, month }) => [{ type: 'Report' as const, id: `monthly-${year}-${month}` }],
        }),

        // Get yearly report
        getYearlyReport: builder.query({
            query: (year) => `/reports/yearly?year=${year}`,
            providesTags: (result, error, year) => [{ type: 'Report' as const, id: `yearly-${year}` }],
        }),
