// app/src/render/billRenderer.ts

export interface BillSchema {
  paperWidth: '58mm' | '80mm';
  dateFormat: 'standard' | 'ISO' | 'short' | 'long';
  timeFormat: '12h' | '24h';
  currency?: string;
...
export interface BillSection {
  ...
  visible: boolean;
  options: any; // Type strictly if needed
}
...
export function renderBill(data: BillData, schema: BillSchema = DEFAULT_BILL_SCHEMA) {
...
function formatDate(dateStr: string, dateFormat: string, timeFormat: string) {
...
function renderSection(section: BillSection, data: BillData, dateFormat: string, timeFormat: string, currency: string) {
  ...
  switch (section.type) {
    ...
    case 'items':
      const rows = data.bill.items.map(item => `
        <tr>
...
function formatCurrency(amount: number | undefined | null, currency = '₹') {
