...
import { useGetTaxProfilesQuery, useGetTaxCategoriesQuery, useGetTaxRulesQuery, useCreateTaxRuleMutation, useDeleteTaxRuleMutation } from '@/services/taxesApi';
...
export default function TaxRules() {
    ...
    const { data: profiles } = useGetTaxProfilesQuery(undefined);
    const { data: categories } = useGetTaxCategoriesQuery(undefined);
    const { data: rules } = useGetTaxRulesQuery(selectedProfileId, { skip: !selectedProfileId });
    ...
    if (!selectedProfileId) {
        // Just render profile selector or loading state?
        // Wait, if profiles exist, select first one? Or wait user?
        // Better let user select.
    }
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                ...
                <Select value={String(selectedProfileId || '')} onValueChange={v => setSelectedProfileId(parseInt(v))}>
                    <SelectTrigger className="w-[200px]"><SelectValue placeholder="Select Profile" /></SelectTrigger>
                    <SelectContent>
                        {profiles?.map((p: any) => (
