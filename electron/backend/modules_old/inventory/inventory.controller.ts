 ...
 */
import * as inventoryService from '../../../../packages/core/src/modules/inventory/inventory.service.js';
 ...
 */
export async function getVariantStockController(req: Request, res: Response) {
    try {
 ...
 */
export async function getVariantLotsController(req: Request, res: Response) {
    try {
        const variantId = parseInt(req.params.id as string, 10);
        if (isNaN(variantId)) {
            return res.status(400).json({ error: 'Invalid variant ID.' });
 ...
 */
export async function getVariantAdjustmentsController(req: Request, res: Response) {
    try {
        const variantId = parseInt(req.params.id as string, 10);
        if (isNaN(variantId)) {
            return res.status(400).json({ error: 'Invalid variant ID.' });
 ...
 */
export async function getLowStockAlertsController(req: Request, res: Response) {
    try {
 ...
 */
export async function getExpiringLotsController(req: Request, res: Response) {
    try {
        const days = getQueryNumber(req.query.days, 30) || 30;
 ...
 */
export async function receiveInventoryController(req: Request, res: Response) {
    try {
        const {
            variantId,
            quantity,
 ...
 */
export async function getInventoryStatsController(req: Request, res: Response) {
    try {
