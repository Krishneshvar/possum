 ...
 */
export function insertReturn({ sale_id, user_id, reason }) {
    const db = getDB();
 ...
 * @returns {Object} Insert result
 */
export function insertReturnItem({ return_id, sale_item_id, quantity, refund_amount }) {
    const db = getDB();
    const stmt = db.prepare(`
 ...
 */
export function findReturnById(id) {
    const db = getDB();
 ...
 */
export function findReturnsBySaleId(saleId) {
    const db = getDB();
    return db.prepare(`
        SELECT 
            r.*,
 ...
 */
export function findReturns({ saleId, userId, startDate, endDate, currentPage = 1, itemsPerPage = 20 }) {
    const db = getDB();
    const filterClauses = [];
    const filterParams = [];
 ...
 */
export function getTotalReturnedQuantity(saleItemId) {
    const db = getDB();
