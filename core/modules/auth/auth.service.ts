...
import { ISessionRepository } from './auth.repository.interface.js';
...
function buildAuthUser(userId: number): AuthUser {
  ...
  const roles = UserRepository.getUserRoles(userId).map((r: any) => r.name);
...
export function getSession(token: string): AuthUser | null {
    ...
    const roles = UserRepository.getUserRoles(session.user_id).map((r: any) => r.name);
