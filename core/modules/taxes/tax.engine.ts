...
import { TaxProfile, TaxRule } from './tax.repository.interface.js';
...
export function initTaxEngine(repo: any) {
    taxRepository = repo;
}
...
export class TaxEngine {
     ...
     */
    init(): void {
        const profile = taxRepository.getActiveTaxProfile();
        this.profile = profile ? (profile as unknown as TaxProfile) : null;

        if (this.profile) {
            this.rules = taxRepository.getTaxRulesByProfileId(this.profile.id) as unknown as TaxRule[];
        } else {
            this.rules = [];
        }
    }
