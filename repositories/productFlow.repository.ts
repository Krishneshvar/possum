...
import type { IProductFlowRepository, FlowQueryOptions } from '../core/index.js';
...
export class ProductFlowRepository implements IProductFlowRepository {
    /**
     ...
     */
    findFlowByVariantId(variantId: number, {
        limit = 100,
        offset = 0,
        startDate,
        endDate,
     ...
     */
    getFlowSummary(variantId: number): ProductFlowSummary {
        const db = getDB();
     ...
     * @returns {Array} Flow events
     */
    findFlowByReference(referenceType: string, referenceId: number) {
        const db = getDB();
        return db.prepare(`
